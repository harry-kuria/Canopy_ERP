...
router.post('/signup',upload.single('photoURL'), (req, res, next)=> {
    ...
    const sqlInsert = 'INSERT INTO other (first_name, last_name, email, password, contact, speciality, brief_speciality, verified, regnumber, photourl,academics,professional,certification,bio,insurance,gender,approved) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);';
    pool.query(sqlInsert,[ first_name,last_name,email, password, contact, speciality,brief_speciality,verified,regnumber, "","","","","","","",""], (error, results) =>{
      ...
      let mailOptions = {
          ...
          to: email,
          subject: 'Dear Specialist. '+first_name+' '+last_name,
...
router.post('/login', (req, res, next)=>{
    ...
    const sqlcheck = 'SELECT * FROM other WHERE email=? AND password=?';
...
router.get('/',(req, res, next)=> {
    ...
    const sqlSelect = "SELECT * FROM other"; 
...
router.post('/load_profile_photo',(req, res, next)=> {
  const email = req.body.email;
  const sqlSelect = "SELECT photourl FROM other WHERE email=?"; 
...
//edit profilePhoto
router.put('/other',upload.single('photoURL'),(req,res, next)=> {
  const email = req.body.email;
  const photoURL = `http://192.168.2.119:5000/${req.file.path}`;

  const sql = 'UPDATE other SET photourl=? WHERE email = ?';
...
router.post('/date_time',(req,res, next)=> {
  ...
  const sql = 'UPDATE other SET date=?, time=? WHERE email = ?';
...
router.post('/add_academics',(req,res, next)=> {
  ...
  const sql = 'UPDATE other SET academics=? WHERE email = ?';
...
router.post('/read_date_time',(req,res, next)=> {
  ...
  const sql = 'SELECT date,time FROM other WHERE email = ?';
...
router.post('/read_bio',(req,res, next)=> {
  ...
  const sql = 'SELECT bio FROM other WHERE email = ?';
  pool.query(sql,[email], (error, result)=>{
...
router.post('/read_academics',(req,res, next)=> {
  ...
  const sql = 'SELECT academics FROM other WHERE email = ?';
  pool.query(sql,[email], (error, result)=>{
...
router.post('/read_professional',(req,res, next)=> {
  ...
  const sql = 'SELECT professional FROM other WHERE email = ?';
  pool.query(sql,[email], (error, result)=>{
...
router.post('/read_certification',(req,res, next)=> {
  ...
  const sql = 'SELECT certification FROM other WHERE email = ?';
  pool.query(sql,[email], (error, result)=>{
...
router.post('/add_professional',(req,res, next)=> {
  ...
  const sql = 'UPDATE other SET professional=? WHERE email = ?';
...
router.post('/add_certification',(req,res, next)=> {
  ...
  const sql = 'UPDATE other SET certification=? WHERE email = ?';
...
router.post('/add_bio',(req,res, next)=> {
  ...
  const sql = 'UPDATE other SET bio=? WHERE email = ?';
...
//get a doctor by ID
router.get('/:otherId', (req, res, next) => {
  const otherId = req.params.otherId;
  const query = 'SELECT * FROM other WHERE id = ?';

  pool.query(query, [otherId], (err, results) => {
    if (err) {
      console.log(err);
      res.status(500).json({
...
router.put('/:id', (req, res) => {
  ...
  pool.query(
    'UPDATE other SET approved = ? WHERE id = ?',
    [approval, id],
    (err, result) => {
      if (err) {
        console.error(err);
...
router.get('/items/count', (req, res) => {
  pool.query('SELECT COUNT(*) as count FROM other', (error, results) => {
    if (error) {
      console.log(error);
...
router.delete('/:doctorId', (req, res, next) => {
  const sql = `DELETE FROM other WHERE id = ${req.params.doctorId}`;
