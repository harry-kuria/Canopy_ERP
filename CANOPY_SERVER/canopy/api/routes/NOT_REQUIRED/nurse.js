...
router.post('/signup',upload.single('photoURL'), (req, res, next)=> {
    ...
    const sqlInsert = 'INSERT INTO nurses (first_name, last_name, email, password, contact, speciality, brief_speciality, verified, regnumber, photourl,academics,professional,certification,bio,insurance,gender,approved) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);';
    pool.query(sqlInsert,[ first_name,last_name,email, password, contact, speciality,brief_speciality,verified,regnumber, "","","","","","","","" ], (error, results) =>{
      ...
      let mailOptions = {
          ...
          to: email,
          subject: 'Dear Nurse. '+first_name+' '+last_name,
...
router.post('/login', (req, res, next)=>{
    ...
    const sqlcheck = 'SELECT * FROM nurses WHERE email=? AND password=?';
...
router.post('/load_profile_photo',(req, res, next)=> {
  const email = req.body.email;
  const sqlSelect = "SELECT photourl FROM nurses WHERE email=?"; 
...
router.post('/update_photo',upload.single('photoURL'),(req,res, next)=> {
  ...
  const sql = 'UPDATE nurses SET photourl=? WHERE email = ?';
...
router.post('/date_time',(req,res, next)=> {
  ...
  const sql = 'UPDATE nurses SET date=?, time=? WHERE email = ?';
...
router.post('/add_academics',(req,res, next)=> {
  ...
  const sql = 'UPDATE nurses SET academics=? WHERE email = ?';
...
router.post('/read_date_time',(req,res, next)=> {
  ...
  const sql = 'SELECT date,time FROM nurses WHERE email = ?';
...
router.post('/read_bio',(req,res, next)=> {
  ...
  const sql = 'SELECT bio FROM nurses WHERE email = ?';
  pool.query(sql,[email], (error, result)=>{
...
router.post('/read_academics',(req,res, next)=> {
  ...
  const sql = 'SELECT academics FROM nurses WHERE email = ?';
  pool.query(sql,[email], (error, result)=>{
...
router.post('/read_professional',(req,res, next)=> {
  ...
  const sql = 'SELECT professional FROM nurses WHERE email = ?';
  pool.query(sql,[email], (error, result)=>{
...
router.post('/read_certification',(req,res, next)=> {
  ...
  const sql = 'SELECT certification FROM nurses WHERE email = ?';
  pool.query(sql,[email], (error, result)=>{
...
router.post('/add_professional',(req,res, next)=> {
  ...
  const sql = 'UPDATE nurses SET professional=? WHERE email = ?';
...
router.post('/add_certification',(req,res, next)=> {
  ...
  const sql = 'UPDATE nurses SET certification=? WHERE email = ?';
...
router.post('/add_bio',(req,res, next)=> {
  ...
  const sql = 'UPDATE nurses SET bio=? WHERE email = ?';
...
router.put('/:id', (req, res) => {
  ...
  pool.query(
    'UPDATE nurses SET approved = ? WHERE id = ?',
    [approval, id],
    (err, result) => {
      if (err) {
        console.error(err);
...
router.get('/items/count', (req, res) => {
  pool.query('SELECT COUNT(*) as count FROM nurses', (error, results) => {
    if (error) {
      console.log(error);
...
router.delete('/:doctorId', (req, res, next) => {
  const sql = `DELETE FROM nurses WHERE id = ${req.params.doctorId}`;
