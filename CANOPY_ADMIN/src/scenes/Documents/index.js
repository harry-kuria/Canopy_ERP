...
const Team = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("http://18.191.22.39:5000/upload_cv");
  ...
  const columns = [
    ...
    {
      field: "email",
      headerName: "Medic Email Address",
      flex: 1,
...

    {
        ...
        cellClassName: "name-column--cell",
        renderCell: (params) => (
          <button onClick={() => window.open(params.value)}>View Document</button>
        ),
        valueGetter: (params) => params.row.pdf,
      },
  ...
  return (
    <Box m="20px">
      <Header 
  title="DOCUMENTS"
  subtitle="Get to View All Uploaded Documents"
  style={{ color: "aqua" }}
